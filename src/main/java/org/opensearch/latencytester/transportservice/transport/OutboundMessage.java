...
abstract class OutboundMessage extends org.opensearch.latencytester.transportservice.transport.NetworkMessage {
    ...
    static class Response extends OutboundMessage {
        ...
        private static byte setStatus(boolean compress, boolean isHandshake, Writeable message) {
            byte status = 0;
            status = org.opensearch.latencytester.transportservice.transport.TransportStatus.setResponse(status);
            if (message instanceof RemoteTransportException) {
                status = org.opensearch.latencytester.transportservice.transport.TransportStatus.setError(status);
            }
            if (compress) {
                status = org.opensearch.latencytester.transportservice.transport.TransportStatus.setCompress(status);
            }
